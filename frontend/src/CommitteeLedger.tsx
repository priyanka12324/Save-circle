...
export default function CommitteeLedger() {
  ...
  const readyToDistribute = Boolean(ledger?.summary.ready_to_distribute)
  ...
  return <>
    ...
    {ledger && <>
      ...
      {Boolean(selectedGroup) && selectedGroup && <section className="workspace-panel"><div className="panel-heading"><div><span>Financial rules</span><h2>Advance and interest policy</h2></div></div><div className="settlement-grid"><div><small>Normal interest</small><strong>{String(selectedGroup.normal_interest_rate ?? 1)}%</strong></div><div><small>Overdue interest</small><strong>{String(selectedGroup.overdue_interest_rate ?? 2)}%</strong></div><div><small>Repayment period</small><strong>{String(selectedGroup.repayment_period_months ?? 6)} months</strong></div><div><small>Bank interest assumption</small><strong>{String(selectedGroup.bank_interest_rate ?? 0)}% p.a.</strong></div></div></section>}
      ...
      <section className="workspace-panel"><div className="panel-heading"><div><span>Advance / withdrawal activity</span><h2>Requests, approvals and repayments</h2></div></div>{advances.length ? <div className="data-cards">{advances.map(row => {
        const f = repayForm[String(row.id)] || { principal: '', interest: '' }
        const hasDueDate = row.due_date !== null && row.due_date !== undefined && String(row.due_date) !== ''
        const canManage = Boolean(row.can_manage)
        const status = String(row.status)
        return <article key={String(row.id)}><div><span>{String(row.group_name)} · Cycle {String(row.cycle_number)}</span><h3>{String(row.member_name)} · {money(row.amount)}</h3><p>{String(row.reason || 'No reason provided')}</p><small>Status: {status} · Outstanding: {money(row.outstanding_principal)} · Interest rule: {String(row.applied_interest_rate || 0)}% · Expected interest: {money(row.expected_interest)}</small>{hasDueDate ? <small> · Due: {String(row.due_date).slice(0,10)}</small> : null}</div><div className="row-actions"><em className={`state ${status.toLowerCase()}`}>{status}</em>{canManage && status === 'PENDING' ? <><button className="approve" onClick={() => decide(row.id, 'APPROVED')}>Approve</button><button className="reject" onClick={() => decide(row.id, 'REJECTED')}>Reject</button></> : null}{canManage && ['APPROVED','OVERDUE'].includes(status) ? <div className="advance-repay"><input type="number" placeholder="Principal" value={f.principal} onChange={e => setRepayForm({ ...repayForm, [String(row.id)]: { ...f, principal: e.target.value } })}/><input type="number" placeholder="Interest" value={f.interest} onChange={e => setRepayForm({ ...repayForm, [String(row.id)]: { ...f, interest: e.target.value } })}/><button className="approve" onClick={() => repay(row)}>Record Repayment</button></div> : null}</div></article>
      ...
      <section className="workspace-panel member-settlement-card">
        <div className="panel-heading"><div><span>My calculated settlement</span><h2>{String(ledger.member_summary.member_name)}</h2></div><em className={`state ${readyToDistribute ? 'verified' : 'pending'}`}>{readyToDistribute ? 'READY TO DISTRIBUTE' : 'SETTLEMENT PENDING'}</em></div>
