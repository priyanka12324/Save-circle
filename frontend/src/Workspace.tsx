...
function Groups({rows,isAdmin,user,changed,fail}:{rows:JsonRecord[];isAdmin:boolean;user:User;changed:(m:string)=>void;fail:(m:string)=>void}){
  ...
  const[form,setForm]=useState({name:'',description:'',contribution_amount:'2000',contribution_frequency:'Monthly',max_members:'10',total_cycles:'12',normal_interest_rate:'1',overdue_interest_rate:'2',repayment_period_months:'6',bank_interest_rate:'0'})
  const create=async()=>{try{await api('/api/groups',{method:'POST',body:JSON.stringify({...form,contribution_amount:Number(form.contribution_amount),max_members:Number(form.max_members),total_cycles:Number(form.total_cycles),normal_interest_rate:Number(form.normal_interest_rate),overdue_interest_rate:Number(form.overdue_interest_rate),repayment_period_months:Number(form.repayment_period_months),bank_interest_rate:Number(form.bank_interest_rate)})});setShow(false);setForm({...form,name:'',description:''});changed(isAdmin?'Savings group created.':'Savings group created — you are now its Group Creator.')}catch(e){fail(e instanceof Error?e.message:'Unable to create group')}}
  ...
  return <><div className="section-actions"><button className="primary-action" onClick={()=>setShow(!show)}>+ Create Savings Group</button></div>{show&&<div className="action-form"><h3>Create your savings group</h3><p>{isAdmin?'Create a platform-managed savings circle and define its financial rules.':'You become the Group Creator. Set the duration, contribution and advance-interest rules before starting.'}</p><div className="form-grid"><label>Group name<input value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label>Contribution amount<input type="number" value={form.contribution_amount} onChange={e=>setForm({...form,contribution_amount:e.target.value})}/></label><label>Frequency<select value={form.contribution_frequency} onChange={e=>setForm({...form,contribution_frequency:e.target.value})}><option>Monthly</option><option>Weekly</option><option>Bi-Weekly</option></select></label><label>Max members<input type="number" value={form.max_members} onChange={e=>setForm({...form,max_members:e.target.value})}/></label><label>Total cycles<input type="number" min="1" value={form.total_cycles} onChange={e=>setForm({...form,total_cycles:e.target.value})}/></label><label>Normal advance interest (%)<input type="number" min="0" step="0.1" value={form.normal_interest_rate} onChange={e=>setForm({...form,normal_interest_rate:e.target.value})}/></label><label>Overdue interest (%)<input type="number" min="0" step="0.1" value={form.overdue_interest_rate} onChange={e=>setForm({...form,overdue_interest_rate:e.target.value})}/></label><label>Repayment period (months)<input type="number" min="1" value={form.repayment_period_months} onChange={e=>setForm({...form,repayment_period_months:e.target.value})}/></label><label>Bank interest rate (% p.a.)<input type="number" min="0" step="0.01" value={form.bank_interest_rate} onChange={e=>setForm({...form,bank_interest_rate:e.target.value})}/></label><label className="wide">Description<input value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></label></div><p className="ledger-note">Committee interest is applied to approved advances. Bank interest is a group assumption/reference; actual credited bank interest should be recorded separately.</p><button className="primary-action" onClick={create}>Create Group</button></div>}{rows.length?<section className="card-grid">{rows.map(r=><article key={value(r,'id')}><div className="card-icon">{Boolean(r.is_creator)?<Crown size={22}/>:<Landmark size={22}/>}</div><span>{Boolean(r.is_creator)?'GROUP CREATOR':value(r,'contribution_frequency')}</span><h3>{value(r,'name')}</h3><p>{value(r,'description','Community savings group')}</p><dl><div><dt>Contribution</dt><dd>{money(r.contribution_amount)}</dd></div><div><dt>Members</dt><dd>{value(r,'member_count')}</dd></div><div><dt>Total saved</dt><dd>{money(r.total_collected)}</dd></div><div><dt>Cycle</dt><dd>{value(r,'current_cycle')} / {value(r,'total_cycles')}</dd></div><div><dt>Advance interest</dt><dd>{value(r,'normal_interest_rate','1')}%</dd></div><div><dt>Overdue interest</dt><dd>{value(r,'overdue_interest_rate','2')}%</dd></div></dl>{!isAdmin&&!Boolean(r.is_member)&&!Boolean(r.is_creator)&&<button className="join-button" onClick={()=>join(r.id)}>Join Group</button>}{Boolean(r.is_creator)&&<div className="creator-note"><Crown size={14}/> You manage this group · contributions and advance requests require review</div>}</article>)}</section>:<Empty text="No active savings groups were found."/>}</>
}

function Members({rows}:{rows:JsonRecord[]}){return rows.length?<Table headers={['Member','Email','Phone','Role','Status']} rows={rows.map(r=>[value(r,'full_name'),value(r,'email'),value(r,'phone'),value(r,'role'),Boolean(r.is_active)?'Active':'Inactive'])}/>:<Empty text="No members were found."/>}
